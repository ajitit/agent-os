...
export default function SkillRegistryPage() {
  ...
  const load = async () => {
    try {
      const [s, g] = await Promise.all([
        api.get<{ data: Skill[] }>("/registry/skills"),
        api.get<{ data: SkillGroup[] }>("/registry/skills/groups"),
      ]);
      setSkills(s.data);
      setGroups(g.data);
  ...
  const openEdit = (s: Skill) => {
    setEditSkill(s);
    setForm({ name: s.name, description: s.description, category: s.category, author: s.author || "", version: s.version, status: s.status, tags: (s.tags || []).join(", ") });
    setError(""); setShowModal(true);
  };
  ...
  const saveSkill = async () => {
    ...
    try {
      if (editSkill) { await api.put(`/registry/skills/${editSkill.id}`, body); }
      else { await api.post("/registry/skills", body); }
      setShowModal(false); load();
    } catch (e: unknown) { setError(e instanceof Error ? e.message : "Failed to save."); }
  };

  const deleteSkill = async (id: string) => {
    try { await api.delete(`/registry/skills/${id}`); setDeleteId(null); load(); } catch { /* ignore */ }
  };

  const saveGroup = async () => {
    if (!groupForm.name.trim()) return;
    try { await api.post("/registry/skills/groups", groupForm); setShowGroupModal(false); setGroupForm({ name: "", description: "" }); load(); } catch { /* ignore */ }
  };

  const deleteGroup = async (id: string) => {
    try { await api.delete(`/registry/skills/groups/${id}`); load(); } catch { /* ignore */ }
  };
      ...
      <div className="mx-auto max-w-7xl px-6 py-8">
        <div className="flex items-center justify-between mb-8">
          <div>
            ...
            <p className="text-zinc-500 text-sm mt-1">Manage skills, organize groups, and assign to agents</p>
          </div>
          <button onClick={openCreate} className="bg-violet-600 hover:bg-violet-700 text-white px-4 py-2 rounded-lg text-sm font-medium transition-colors">+ New Skill</button>
        </div>

        <div className="flex gap-6">
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-3 mb-4">
              <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search skills..." className="flex-1 bg-zinc-900 border border-zinc-800 rounded-lg px-3 py-2 text-sm text-zinc-100 placeholder-zinc-600 focus:outline-none focus:border-zinc-600" />
              <span className="text-zinc-500 text-sm">{filtered.length} skills</span>
            </div>
            {loading ? (
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-zinc-800 bg-zinc-900/50">
                      ...
                      <th className="text-left px-4 py-3 text-zinc-400 font-medium">Tags</th>
                      <th className="px-4 py-3"></th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-zinc-800/50">
                    {filtered.length === 0 && <tr><td colSpan={6} className="px-4 py-8 text-center text-zinc-600">No skills found</td></tr>}
                    {filtered.map(s => (
                      <tr key={s.id} className="hover:bg-zinc-900/50 transition-colors">
                        <td className="px-4 py-3">
                          <div className="font-medium text-zinc-100">{s.name}</div>
                          <div className="text-zinc-500 text-xs mt-0.5 truncate max-w-xs">{s.description}</div>
                        </td>
                        <td className="px-4 py-3 text-zinc-400">{s.category}</td>
                        <td className="px-4 py-3 text-zinc-500 font-mono text-xs">{s.version}</td>
                        <td className="px-4 py-3">
                          <span className={`inline-flex rounded-full px-2 py-0.5 text-xs font-medium ${STATUS_COLORS[s.status] ?? STATUS_COLORS.draft}`}>{s.status}</span>
                        </td>
                        <td className="px-4 py-3">
                          <div className="flex flex-wrap gap-1">{(s.tags || []).slice(0, 3).map(t => <span key={t} className="bg-zinc-800 text-zinc-400 text-xs px-1.5 py-0.5 rounded">{t}</span>)}</div>
                        </td>
                        <td className="px-4 py-3">
                          <div className="flex items-center gap-2 justify-end">
                            <button onClick={() => openEdit(s)} className="text-zinc-500 hover:text-zinc-200 text-xs px-2 py-1 rounded hover:bg-zinc-800 transition-colors">Edit</button>
                            {deleteId === s.id ? (
                              <div className="flex items-center gap-1">
                                <button onClick={() => deleteSkill(s.id)} className="text-red-400 hover:text-red-300 text-xs px-2 py-1 rounded hover:bg-red-900/20">Confirm</button>
                                <button onClick={() => setDeleteId(null)} className="text-zinc-500 text-xs px-2 py-1 rounded hover:bg-zinc-800">Cancel</button>
                              </div>
                            ) : (
                              <button onClick={() => setDeleteId(s.id)} className="text-zinc-600 hover:text-red-400 text-xs px-2 py-1 rounded hover:bg-zinc-800 transition-colors">Delete</button>
                            )}
                          </div>
                        </td>
                      </tr>
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-zinc-900 border border-zinc-800 rounded-2xl w-full max-w-lg shadow-2xl">
            <div className="flex items-center justify-between px-6 py-4 border-b border-zinc-800">
              <h2 className="text-base font-semibold text-white">{editSkill ? "Edit Skill" : "New Skill"}</h2>
            ...
            <div className="p-6 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="col-span-2">
                  ...
                  <input value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))} className="w-full bg-zinc-800 border border-zinc-700 rounded-lg px-3 py-2 text-sm text-zinc-100 focus:outline-none focus:border-violet-500" placeholder="my-skill" />
                </div>
                <div className="col-span-2">
                  <label className="block text-xs text-zinc-400 mb-1.5">Description *</label>
                  <textarea value={form.description} onChange={e => setForm(f => ({ ...f, description: e.target.value }))} rows={2} className="w-full bg-zinc-800 border border-zinc-700 rounded-lg px-3 py-2 text-sm text-zinc-100 focus:outline-none focus:border-violet-500 resize-none" placeholder="What does this skill do?" />
                </div>
                <div>
                ...
                <div className="col-span-2">
                  ...
                  <input value={form.tags} onChange={e => setForm(f => ({ ...f, tags: e.target.value }))} className="w-full bg-zinc-800 border border-zinc-700 rounded-lg px-3 py-2 text-sm text-zinc-100 focus:outline-none focus:border-violet-500" placeholder="nlp, research, web" />
                </div>
              </div>
            </div>
            <div className="flex justify-end gap-2 px-6 py-4 border-t border-zinc-800">
              ...
              <button onClick={saveSkill} className="px-4 py-2 rounded-lg text-sm bg-violet-600 hover:bg-violet-700 text-white font-medium transition-colors">{editSkill ? "Save Changes" : "Create Skill"}</button>
            </div>
          </div>
        </div>
      )}
      ...
      {showGroupModal && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-zinc-900 border border-zinc-800 rounded-2xl w-full max-w-md shadow-2xl">
            <div className="flex items-center justify-between px-6 py-4 border-b border-zinc-800">
              <h2 className="text-base font-semibold text-white">New Skill Group</h2>
            ...
            <div className="p-6 space-y-4">
              <div>
                ...
                <input value={groupForm.name} onChange={e => setGroupForm(f => ({ ...f, name: e.target.value }))} className="w-full bg-zinc-800 border border-zinc-700 rounded-lg px-3 py-2 text-sm text-zinc-100 focus:outline-none focus:border-violet-500" placeholder="Core Skills" />
              </div>
              <div>
                <label className="block text-xs text-zinc-400 mb-1.5">Description</label>
